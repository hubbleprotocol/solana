...
// Alternative to ProcessInstruction, but with lifetimes compatible with anchor 0.29 entrypoints
// Workaround for https://github.com/coral-xyz/anchor/pull/2711
pub type AnchorProcessInstruction = for<'info> fn(
    program_id: &Pubkey,
    accounts: &'info [AccountInfo<'info>],
    instruction_data: &[u8],
) -> ProgramResult;

pub fn invoke_builtin_anchor_function(
    builtin_function: AnchorProcessInstruction,
    invoke_context: &mut InvokeContext,
) -> Result<u64, Box<dyn std::error::Error>> {
    set_invoke_context(invoke_context);
    ...
    // Lookup table for AccountInfo
    let account_info_map: HashMap<_, _> =
        account_infos.iter().map(|a| (*a.key, a.clone())).collect();
...
/// Converts a `anchor-program`-style entrypoint into the runtime's entrypoint style, for
/// use with `ProgramTest::add_program`
#[macro_export]
macro_rules! anchor_processor {
    ($builtin_function:expr) => {
        Some(|vm, _arg0, _arg1, _arg2, _arg3, _arg4| {
            ...
            vm.program_result = $crate::invoke_builtin_anchor_function(
                $builtin_function,
